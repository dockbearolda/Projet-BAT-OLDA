...
// Barre repliable : une grille taille → quantité. Seules les tailles avec une
// quantité > 0 ET un libellé partent sur le BAT. L'utilisateur peut ajouter des
// tailles personnalisées (ex. « 8 ans », « TU ») et les supprimer.
//
// Le panneau ouvert s'affiche en POPOVER (position absolue, au-dessus des
// mockups) comme les menus réf/couleur : il ne vole pas de hauteur au reste de
// l'écran → les t-shirts ne rétrécissent pas en mode 2 vues « sans scroll ».
export function OrderSizesEditor({
  ...
  // Le compteur d'en-tête est calculé sur les tailles RÉELLEMENT exportées
  // (agrégées, libellé non vide) → cohérent avec ce qui s'imprime sur le BAT.
  ...
  // Fermeture au clic-extérieur + Échap (même comportement que les dropdowns).
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-40 mt-2 rounded-xl border border-duck/15 bg-white px-4 py-4 shadow-olda ring-1 ring-black/5">
          <div className="flex max-h-[46vh] flex-wrap items-end gap-2.5 overflow-y-auto">
            {sizes.map((s) => {
              const labelMissing = s.qty > 0 && s.label.trim().length === 0;
              return (
                <div
                  key={s.id}
                  className="relative flex w-[68px] flex-col items-stretch gap-1"
                >
                  {s.custom ? (
                    <input
                      type="text"
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg border bg-white py-1.5 text-center text-base font-semibold tabular-nums sm:text-sm focus:outline-none focus:ring-1 focus:ring-duck-focus ${
                      s.qty > 0
                        ? "border-duck/40 text-ink"
                        : "border-duck/15 text-muted2 placeholder:text-muted2/50"
                    }`}
                  />
                  {s.custom && (
                    <button
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            {missing ? (
              ...
              <p className="text-[11px] text-muted2">
                Seules les tailles avec une quantité apparaissent sur le BAT.
              </p>
            )}
            {total > 0 && (
              <button
